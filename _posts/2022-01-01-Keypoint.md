...
$$\min\limits_{C \in \mathbb{R}^{d \times M}} \frac{1}{N} \sum\limits_{i=1}^N \sum\limits_{j=1}^{N_j} \min\limits_{y_i^j \in \lbrace 0,1 \rbrace^M} \lVert f_i^j - Cy_i^j \rVert_2^2$$
